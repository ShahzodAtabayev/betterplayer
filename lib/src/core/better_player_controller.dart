...
class BetterPlayerController {
  ...
  Future setupDataSource(BetterPlayerDataSource betterPlayerDataSource) async {
    ...
    if (betterPlayerDataSource.startAt != null) {
      _startAt = betterPlayerDataSource.startAt!;
      seekTo(betterPlayerDataSource.startAt!);
    }
  }
  ...
  Future<void> seekTo(Duration moment) async {
    ...
    if (moment.inSeconds - _startAt.inSeconds > 1) {
      _betterPlayerDataSource?.addWatchDuration(WatchDuration(
        startAt: _startAt,
        endAt: moment,
      ));
    }
  ...
  void dispose({bool forceDispose = false}) {
    ...
    if (!_disposed) {
      if (videoPlayerController != null) {
        _betterPlayerDataSource?.addWatchDuration(
          WatchDuration(startAt: _startAt, endAt: videoPlayerController!.value.position),
        );
        pause();
        videoPlayerController!.removeListener(_onFullScreenStateChanged);
